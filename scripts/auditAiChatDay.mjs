...
const PROJECT_ID = process.env.AUDIT_PROJECT_ID ?? 'pluto-ef61b';
const UID = process.env.AUDIT_UID ?? 'EahOcjp4slbT6nj8YtgNktOySkD2';
const START_ISO = process.env.AUDIT_START_ISO ?? '2026-04-24T04:30:00Z';
const OUTPUT_STEM = process.env.AUDIT_OUTPUT_STEM ?? 'aiChat_audit_2026-04-24';
const USD_TO_INR = Number(process.env.AUDIT_USD_TO_INR ?? '84.5');

const OUTPUT_CSV = `${OUTPUT_STEM}.csv`;
...
const normalizeModelUsed = ({ modelUsed, modelId }) => {
  const normalizedModelUsed =
    typeof modelUsed === 'string' && modelUsed.trim().length > 0 ? modelUsed.trim() : null;
  ...
  if (normalizedModelUsed === 'flash') {
    return 'gemini-2.5-flash';
  }

  if (normalizedModelUsed === 'flash-lite') {
    return 'gemini-2.5-flash-lite';
  }

  if (normalizedModelUsed === 'nova-micro') {
    return 'nova-micro';
  }

  return normalizedModelUsed;
};

const groupEntriesByRequestId = (entries) => {
  const grouped = {};
...
const main = async () => {
  ...
  const auditLogs = await listLogEntries(
    token,
    `timestamp >= "${START_ISO}" AND (` +
      `jsonPayload.eventType="ai_input_context_fit" OR ` +
      `jsonPayload.eventType="ai_route_selected" OR ` +
      `jsonPayload.eventType="ai_attempt_finished" OR ` +
      `jsonPayload.eventType="ai_model_request_failed" OR ` +
      `jsonPayload.eventType="ai_request_completed" OR ` +
      `jsonPayload.eventType="ai_fallback_triggered" OR ` +
      `jsonPayload.eventType="gemini_generate_content_failed" OR ` +
      `jsonPayload.eventType="gemini_503_spike_detected"` +
    `)`
  );
  const broaderGemini503Logs = auditLogs.filter(
    (entry) => getEntryPayload(entry)?.eventType === 'gemini_generate_content_failed'
  );
  const gemini503SpikeLogs = auditLogs.filter(
    (entry) => getEntryPayload(entry)?.eventType === 'gemini_503_spike_detected'
  );
  const flashLiteLogs = broaderGemini503Logs.filter((entry) => {
    const payload = getEntryPayload(entry);
    return payload?.model === 'gemini-2.5-flash-lite' || payload?.modelUsed === 'flash-lite';
  });
  const requestLogGroups = groupEntriesByRequestId(auditLogs);

  const requestRows = [];
  const jsonRequests = [];
  const requestLogsById = {};
  const requestFirestoreById = {};

  for (const request of requests) {
    const requestId = request.requestId;
    const entries = requestLogGroups[requestId] ?? [];
    requestLogsById[requestId] = entries;
    requestFirestoreById[requestId] = request;
    ...
    const fallback = getLatestLogValue(entries, 'ai_fallback_triggered');
    ...
    const modelId =
      ...
      null;
    const modelUsed = normalizeModelUsed({
      modelUsed: completion?.modelUsed ?? attemptSuccess?.modelUsed ?? geminiFailure?.modelUsed ?? request?.response?.modelUsed ?? null,
      modelId,
    });
    ...
    const record = {
      ...
      primaryProvider: route?.primaryProvider ?? null,
      finalProvider: completion?.finalProvider ?? null,
      fallbackTriggered: completion?.fallbackTriggered ?? fallback?.fallbackTriggered ?? false,
      totalRetryCount: completion?.totalRetryCount ?? (Number.isFinite(attempt) ? Math.max(Number(attempt) - 1, 0) : null),
      firestore: request,
      logs: entries,
    };
