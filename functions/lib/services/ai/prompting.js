...
export const buildTurnSpecificInstruction = ({ mode, prompt, history, }) => {
    ...
    if (directAnswerRequest) {
        lines.push('- The latest student message is asking for the answer directly.', '- Do NOT provide the complete solution or final answer in this turn.', '- Reply with exactly one hint or one next step only, and keep the student doing the work.', '- If the student has not already shown two genuine attempts with working, a full solution is forbidden.');
    }
    else if (priorAttemptCount < 2) {
        lines.push('- The student has not yet earned a full worked solution.', '- Stay in hint-first tutoring mode and reveal at most one next step beyond the student\'s current progress.');
    }
    else {
        lines.push('- Even if the student has attempted multiple times, only reveal a full solution if they are still completely unable to proceed.', '- If you reveal a solution, keep it concise and finish with: "Now try a similar problem on your own".');
    }
    return lines.join('\n');
};
