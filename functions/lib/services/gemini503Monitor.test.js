import { GEMINI_503_SPIKE_THRESHOLD, GEMINI_503_WINDOW_MS, getGemini503WindowBucket, recordGemini503, } from './gemini503Monitor.js';
const createFakeDb = () => {
    const store = new Map();
...
test('503 monitor emits spike state at threshold with affected users', async () => {
    ...
    const now = 300_000;
    let result = await recordGemini503('uid0', 'request0', { db: db, now });
...
test('503 monitor resets when the five minute bucket changes', async () => {
    const db = createFakeDb();
    const now = 600_000;
    await recordGemini503('uid1', 'request1', { db: db, now });
