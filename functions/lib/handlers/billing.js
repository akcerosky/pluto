...
export const billingCheckoutHandler = async (request) => {
...
export const billingVerifyPaymentHandler = async (request) => {
    const uid = assertAuth(request);
    ...
    const periodStart = typeof subscription.current_start === 'number'
        ? new Date(subscription.current_start * 1000)
        : getIstNow();
    const periodEnd = typeof subscription.current_end === 'number'
...
export const billingHistoryHandler = async (request) => {
    const uid = assertAuth(request);
    return {
        history: await listPaymentHistory(uid),
    };
};
export const billingSubscriptionGetHandler = async (request) => {
    const uid = assertAuth(request);
    const snapshot = await getMeSnapshot(uid, getBootstrapIdentity(request));
    return {
        subscription: snapshot.subscription,
...
export const billingSubscriptionResumeHandler = async (request) => {
    const uid = assertAuth(request);
    const privateSubscription = await getSubscriptionPrivate(uid);
    if (!privateSubscription.providerSubscriptionId) {
    ...
    if (snapshot.subscription.status === 'cancelled') {
        throw new HttpsError('failed-precondition', 'This subscription cannot be resumed because it was cancelled, not paused. Please subscribe again.');
    }
    try {
        await resumeRazorpaySubscription(privateSubscription.providerSubscriptionId);
    }
    catch (error) {
        const description = typeof error === 'object' && error && 'description' in error ? String(error.description) : '';
...
export const billingRequestRefundHandler = async (request) => {
    const uid = assertAuth(request);
...
export const adminSyncPaymentHandler = async (request) => {
    assertAdmin(request);
    ...
    const refreshedPayment = payment.paymentId ? await fetchRazorpayPayment(payment.paymentId) : null;
    ...
    await createPaymentRecord(payload.uid, payload.paymentRecordId, {
        ...payment,
        status: refreshedPayment?.status === 'captured'
            ? 'captured'
