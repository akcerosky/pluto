...
test('mode output token budgets clamp against plan maximums', () => {
    expect(getEffectiveMaxOutputTokens('Conversational', PLAN_DEFINITIONS.Pro)).toBe(2000);
    expect(getEffectiveMaxOutputTokens('Homework', PLAN_DEFINITIONS.Pro)).toBe(4000);
    expect(getEffectiveMaxOutputTokens('ExamPrep', PLAN_DEFINITIONS.Pro)).toBe(1500);
