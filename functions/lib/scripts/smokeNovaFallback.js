...
const run = async () => {
    ...
    try {
        ...
        if (result.finalProvider !== 'gemini' || !result.fallbackTriggered) {
            throw new Error(`Expected Gemini fallback after Nova failure but received finalProvider=${result.finalProvider}, fallbackTriggered=${String(result.fallbackTriggered)}`);
        }
        if (!result.text.trim()) {
