 ...
 */
const baseFunctionFilter = 'resource.type="cloud_function" OR resource.type="cloud_run_revision"';
export const NOVA_LITE_ALERT_RUNBOOK = [
    {
        name: 'Nova Lite success rate below 95%',
        purpose: 'Detect attachment-path degradation when Nova Lite success volume drops relative to started attempts.',
...
jsonPayload.eventType="nova_lite_success"`,
        ...
        thresholdInterpretation: 'Create two counter metrics: one for nova_lite_success and one for nova_lite_attempt_started. In Cloud Monitoring, alert when success_count / started_count < 0.95 over 10 minutes.',
    ...
    },
    {
        ...
        purpose: 'Catch increased fallback churn before Nova Lite is trusted as the primary multimodal path.',
...
jsonPayload.eventType="gemini_fallback_triggered_from_nova_lite"`,
        timeWindow: '10 minutes',
        thresholdInterpretation: 'Create one counter metric for gemini_fallback_triggered_from_nova_lite and divide it by nova_lite_attempt_started count. Alert when fallback_count / started_count > 0.10.',
        metricType: 'logs-based counter',
        notes: [
...
jsonPayload.latencyMs:*`,
        timeWindow: '10 minutes',
        thresholdInterpretation: 'Create a logs-based distribution metric extracting jsonPayload.latencyMs, then alert when p95(latencyMs) > 30000 over the aligned window.',
...
jsonPayload.providerStatus<600`,
        ...
        thresholdInterpretation: 'Create a counter metric for nova_lite_failed filtered to providerStatus 5xx and divide it by nova_lite_attempt_started count. Alert when 5xx_failed_count / started_count > 0.05.',
        metricType: 'logs-based counter',
        notes: [
    ...
    },
    {
        ...
        purpose: 'Detect token-accounting drift when provider usage is missing and the fallback estimator is being used unusually often.',
        condition: 'Alert if usageAnomaly appears more than 5 times in a 10 minute window.',
        filter: `${baseFunctionFilter}
jsonPayload.eventType="nova_lite_success"
jsonPayload.usageAnomaly:*`,
        timeWindow: '10 minutes',
        thresholdInterpretation: 'Create a counter metric on nova_lite_success entries where jsonPayload.usageAnomaly is present. Alert when count > 5 over 10 minutes.',
        metricType: 'logs-based counter',
        notes: [
