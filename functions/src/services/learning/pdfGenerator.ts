...
const estimateLineWidth = (text: string, fontSize: number) => text.length * fontSize * 0.48;

const estimateCharsPerLine = (fontSize: number, width: number) =>
  Math.max(16, Math.floor(width / Math.max(fontSize * 0.48, 1)));
...
const buildRenderBlocks = (paper: QuestionPaperDoc): RenderBlock[] => {
...
const buildPageContentStreams = (paper: QuestionPaperDoc) => {
  ...
  const drawTextLine = ({
    ...
    const x =
      align === 'center'
        ? Math.max(
            MARGIN_X,
            MARGIN_X + (CONTENT_WIDTH - estimateLineWidth(text, fontSize)) / 2
          )
        : MARGIN_X + indent;
  ...
  for (const block of buildRenderBlocks(paper)) {
    ...
    const requiredHeight =
      lines.length * block.lineHeight + (block.spacingAfter ?? 0);
