...
const estimateLineWidth = (text: string, fontSize: number) => text.length * fontSize * 0.5;

const wrapText = (value: string, fontSize: number, width: number) => {
...
const drawLine = (page: PdfPage, x1: number, y1: number, x2: number, y2: number, lineWidth = 1) => {
...
const drawRect = (page: PdfPage, x: number, y: number, width: number, height: number, lineWidth = 1) => {
  page.commands.push('0 0 0 RG');
  page.commands.push(`${lineWidth} w`);
...
const sectionDisplayTitle = (section: QuestionPaperFormatSection) => {
...
const getHeaderSessionLabel = (paper: QuestionPaperDoc) => sanitizePdfRenderableText(paper.sessionLabel || String(new Date().getFullYear()));
const getHeaderBoardName = (paper: QuestionPaperDoc) => sanitizePdfRenderableText(paper.headerBoardName || paper.examBoard);
const getHeaderExamTitle = (paper: QuestionPaperDoc) =>
  sanitizePdfRenderableText(paper.examinationTitle || `${paper.educationLevel} Examination`);
const getSubjectCode = (paper: QuestionPaperDoc) => sanitizePdfRenderableText(paper.subjectCode || '__________');

const getGeneralInstructions = (paper: QuestionPaperDoc) =>
...
const buildQuestionBlockLines = (question: QuestionPaperQuestion) => {
  const baseLine = `${question.questionNumber}. ${sanitizePdfRenderableText(question.text)}`;
  const optionLines =
    question.options?.map((option, index) => `(${String.fromCharCode(65 + index)}) ${sanitizePdfRenderableText(option)}`) ?? [];
  const subPartLines =
    question.subParts?.map((part, index) => `(${String.fromCharCode(97 + index)}) ${sanitizePdfRenderableText(part)}`) ?? [];
...
const getSectionMarkingScheme = (section: QuestionPaperFormatSection) => {
...
const isNumericalQuestion = (question: QuestionPaperQuestion, section?: QuestionPaperFormatSection) =>
  question.type === 'numerical' ||
  question.type === 'integer' ||
  /numerical|integer/i.test(section?.questionType || '') ||
  /numerical|integer/i.test(section?.questionTypeDisplay || '');

const estimateQuestionBlockHeight = (question: QuestionPaperQuestion) => {
...
const ensureSpace = (pages: PdfPage[], requiredHeight: number) => {
...
const renderHeader = (page: PdfPage, paper: QuestionPaperDoc) => {
  const boxHeight = 150;
...
const renderGeneralInstructions = (page: PdfPage, paper: QuestionPaperDoc) => {
...
const renderSectionHeader = (page: PdfPage, section: QuestionPaperFormatSection) => {
...
const renderQuestion = (page: PdfPage, question: QuestionPaperQuestion, section?: QuestionPaperFormatSection) => {
  const { baseLine, optionLines, subPartLines } = buildQuestionBlockLines(question);
  const baseLines = wrapText(baseLine, 11, CONTENT_WIDTH - MARKS_GUTTER);
...
const renderFooter = (page: PdfPage, paper: QuestionPaperDoc, pageNumber: number, totalPages: number) => {
...
const buildPageContentStreams = (paper: QuestionPaperDoc) => {
...
export const generateQuestionPaperPdfBase64 = (paper: QuestionPaperDoc) => {
  ...
  objects.push(
    `2 0 obj << /Type /Pages /Kids [${pageObjectNumbers.map((number) => `${number} 0 R`).join(' ')}] /Count ${pageObjectNumbers.length} >> endobj`
  );
  ...
  for (const [index, stream] of contentStreams.entries()) {
    ...
    objects.push(
      `${pageObjectNumber} 0 obj << /Type /Page /Parent 2 0 R /MediaBox [0 0 ${PAGE_WIDTH} ${PAGE_HEIGHT}] /Resources << /Font << /F1 3 0 R /F2 4 0 R >> >> /Contents ${contentObjectNumber} 0 R >> endobj`
    );
    objects.push(
