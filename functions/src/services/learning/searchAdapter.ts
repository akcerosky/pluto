...
export const searchExamFormatSources = async (query: string): Promise<SearchResult[]> => {
  ...
  const titles = Array.from(
    html.matchAll(/<a[^>]*class="result__a"[^>]*href="([^"]+)"[^>]*>(.*?)<\/a>/gsi)
  ).slice(0, 5);
  const snippets = Array.from(
    html.matchAll(/<a class="result__snippet"[^>]*>(.*?)<\/a>/gsi)
  ).slice(0, 5);
