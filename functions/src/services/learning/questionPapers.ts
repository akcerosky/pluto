...
const normalizeFormatFallback = ({
  ...
  const common = {
    ...
    matchedFormatFamily,
    formatSource: 'family_fallback' as const,
  };
...
${formatHint.sections
  .map(
    (section) => `## ${section.displayName || section.name} — ${section.questionTypeDisplay || section.questionType} (${section.questions} × ${section.marksPerQuestion} = ${
