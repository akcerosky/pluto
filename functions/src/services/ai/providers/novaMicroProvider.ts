...
const normalizeHistory = (history: AiHistoryMessage[]) => {
  const sanitized: Array<{ role: 'user' | 'assistant'; content: string }> = history
    .map((message) => ({
      role: message.role,
  ...
  return sanitized.reduce<Array<{ role: 'user' | 'assistant'; content: string }>>((acc, message) => {
...
const validateNovaText = (text: string) => {
  if (!text.trim()) {
...
const refreshContextSummary = async (
  request: ProviderRequest,
  systemInstruction: string
): Promise<ThreadContextSummary | undefined> => {
  ...
  const fallbackSummary: ThreadContextSummary = {
    ...
    summarizedMessageCount:
      (request.contextSummary?.summarizedMessageCount ?? 0) + request.summaryCandidates.length,
    summarizedExchangeCount:
      (request.contextSummary?.summarizedExchangeCount ?? 0) + Math.ceil(request.summaryCandidates.length / 2),
    blockSize: 10,
    updatedAt: Date.now(),
  };
...
export const isRetryableNovaError = (error: unknown) => {
  const details = getProviderErrorDetails(error);
...
export const generateNovaMicroResponse = async (request: ProviderRequest): Promise<ProviderResult> => {
