...
const assertFirestoreAccessConfigured = () => {
  ...
  if (!hasEmulator && !hasCredentials) {
    throw new Error(
      'smokeDeleteThread requires either FIRESTORE_EMULATOR_HOST for local emulator runs or admin credentials via GOOGLE_APPLICATION_CREDENTIALS / FIREBASE_ADMIN_REFRESH_TOKEN_JSON.'
    );
  }
};

const makeThreadRef = (threadId: string) =>
...
const run = async () => {
  ...
  if (threadSnapshot.exists || !messagesSnapshot.empty) {
    throw new Error(
      `Smoke delete failed for ${threadId}: threadExists=${threadSnapshot.exists}, remainingMessages=${messagesSnapshot.size}`
    );
  }
